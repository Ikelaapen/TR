...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $insert_query = $conn->prepare("INSERT INTO pelunasan (id_penghuni, nama, tglbayar, tagihan, status, catatan) 
                                VALUES (?, ?, ?, ?, ?, ?)");
$insert_query->bind_param("isssss", $row['id_penghuni'], $row['nama'], $row['tglbayar'], $row['tagihan'], $row['status'], $row['catatan']);
